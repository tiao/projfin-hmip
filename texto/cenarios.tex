...
de operação por tunelamento bidirecional do MIPv6, sem a otimização de rota. A
...
gerar
tráfego. Para realizar esta tarefa é utilizado a ferramenta \textit{netcat6},
da forma:
\begin{verbatim}
...
OK'', para responder as requisições ao servidor;
 ...
 \item[cat header \texttt{/dev/zero}:] concatena para a entrada padrão do servidor o
arquivo contendo a mensagem HTTP e o dispositivo caracter \texttt{/dev/zero} que é
responsável por gerar o tráfego.
 \end{description}

O nó correspondente irá se conectar ao servidor utilizando da ferramenta 
\textit{wget}. Também no nó correspondente executamos a ferramenta 
...
\subsection{Subsídios para interpretação dos pacotes IPv6 no \textit{tcpdump}}

Nas listagens do \textit{tcpdump}, utilizadas na seções que se seguem, os
pacotes serão mostrados da seguinte forma:
...
O \textit{tcpdump} apresenta os cabeçalhos de mobilidade (BU e BA) com
\textit{next-header unknown} no caso do BU e de \textit{source routing}
no caso do BA. Entretanto, o \textit{next-header} é detalhado na sequência
...
número nos balões cinza representam a VLAN no \textit{uml\_switch}.
...
RA do Agente domiciliar (src = fe80::fcfd:f5ff:fe49:32bc);
 \item O nó móvel realiza o teste de duplicação de endereço (DAD) do CoA gerado
...
\textbf{Destino} & \textbf{Via} & \textbf{Próximo
...
Após o agente domiciliar receber o BU, enviado pelo
nó móvel para efetuar o registro, é adicionada na sua tabela de roteamento
uma rota para que todos os pacotes com destino ao endereço domiciliar sejam
encaminhados para o túnel.
Após o nó móvel receber o RA na rede visitada, o
roteador padrão é alterado. é deletada a rota para rede 2001:c::/64 e adicionada
uma rota para a rede 2001:d::/64, onde a interface eth0 está atrelada.
 
Desde as versões do \textit{kernel linux} 2.x pode-se ter múltiplas tabelas 
de roteamento no kernel. Estas tabelas são identificadas por um número variando
de 1 a 255 ou por um nome de acordo com o arquivo \texttt{/etc/iproute2/rt\_tables}.
Com novas tabelas pode-se criar uma estrutura flexível para implementar uma
Politica de Roteamento \cite{lyra:crl}.

Existem três tabelas que são criadas pelo \textit{kernel}. A \textit{local}(255), é uma tabela de 
roteamento especial mantida pelo kernel, onde usuários não podem adicionar novas
entradas nessa tabela. A \textit{main}(254), é a tabela de roteamento principal, onde ficam
as rotas quando adicionamos com o comando \texttt{route} ou \texttt{ip route}.
A \textit{default}(253) é outra tabela especial e a \textit{unspec}(0), qualquer operações sobre esta
tabela refletem em todas as tabelas.

O nó móvel adiciona regras na tabela 252, mostradas na tabela
\ref{t_route_252_mn}, para que os pacotes com destino ao nó correspondente e ao 
agente domiciliar sejam encaminhados via túnel.
...
com destino ao nó móvel(2001:c::2) e origem no agente domiciliar(2001:c::1) sejam
...
O Banco de Dados da Política de Roteamento ou \ac{RPDB} 
controla a ordem na qual o \textit{kernel} faz sua busca através das tabelas de roteamento.
Cada regra possui uma prioridade, e as regras são examindadas sequencialmente da
regra 0 até a regra 32767.
...
\end{tabular} 
\end{small}
\caption{Regras de Roteamento do Nó móvel}
\label{t_rule_mn}
\end{table} 
...
\ref{t_tun_ha1} e \ref{t_tun_mn1} apresentam o endereçamento do túnel, no HA e 
...
\end{tabular}
\end{small}
\caption{Descrição dos túneis do nó móvel}
\label{t_tun_mn1}
\end{table}
...
\end{tabular}
\end{small}
\caption{Descrição dos túneis do agente domiciliar}
\label{t_tun_ha1}
\end{table}
...
%6
 \item O nó móvel inicia o processo de \acf{RRP} enviando
...
%8
 \item O nó móvel da continuação ao processo de RRP
enviando a mensagem de \textit{Care-of Test Init}(CoTi) ao nó correspondente,
para adquirir a \textit{care-of keygen token}. Essa mensagem não passa pelo 
túnel, ela é endereçada diretamente ao nó correspondente.
%9
 \item O nó móvel recebe a mensagem de \textit{Care-of Test}(CoT) do nó 
correspondente, não passando pelo agente domiciliar.
%10
 \item O nó móvel envia um BU ao nó correspondente com o
...
\subsubsection{Detalhes da Formação de Túneis}
Sobre os túneis, não há nenhuma alteração em relação ao cenário configurado com
...
na função de agente domiciliar, e dois roteadores de acesso (AR2 e AR3), que
...
VLAN 5, domínio do MAP1, caracterizando uma mobilidade global.
...
\begin{figure}[!htpb]
	...
	\caption{Cenário 2: Captura de mensagens Mobilidade Global}
 	\label{f_dump_inter}
\end{figure}
...
\begin{figure}[!htpb]
	...
	\caption{Cenário 2: Captura de mensagens Mobilidade Local}
 	\label{f_dump_intra}
\end{figure}
...
(2001:e::fcfd:32ff:feff:42c9) e o MAP1 (2001:c::1). Este túnel é necessário, pois
os pacotes oriundos do Agente domiciliar serão endereçados agora ao
RCoA(2001:c::fcfd:32ff:feff:42c9). Como no MAP1 há uma associação LCoA-RCOA, este
túnel passa a encaminhar os pacotes do HA ao nó móvel;
 \item para que os pacotes vindos via MAP1 destinados ao endereço domiciliar 
...
do agente domiciliar, e a interface que este túnel está ligado agora deve ser o
outro túnel criado entre nó móvel e o MAP1.
\end{itemize}

Os túneis dos MAP1 e agente domiciliar se mantém iguais aos de um cenário
rodando o MIPv6.
...
O tempo do \textit{handover} é um dos maiores problemas do MIPv6, esta seção do trabalho
pretende fazer uma análise deste processo, apontar as causas e possíveis
soluções. Como base para estudo dos tempos de \textit{handover} foi utilizado o cenário
...
com a saída do teste foi gerado um gráfico, que pode ser observado na figura
\ref{f_banda}.
...
Como o cenário de estudo foi simulado todos os tempos medidos são estimados. Porém,
...
RA. O resultado deste teste esta disponível na figura
\ref{f_radvd}.
...
tentar amenizar a latência do handover, pois um número muito grande de mensagens
...
A Partir, deste teste é possível perceber uma queda de 3 segundos para
...
\textit{handover} o \ac{FMIPv6}. A idéia do FMIPv6 é providenciar
...
perceber continuação da comunicação entre os pontos comunicantes após a
...
funcionamento do HMIPv6, claro que os problemas ainda existentes na implementação
não permitiram comprovar uma melhora no \textit{handover}. Porém, pode-se ver um
avanço já que não existe nenhuma versão recente deste protocolo.

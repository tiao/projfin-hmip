...
mobilidade basta clicar no número da \textit{VLan} a qual queira se associar.
  \item[\textit{Load Scenario}] Por meio deste botão o usuario busca o diretório
...
UML.
 \item Então disponibiliza os recursos do programa ao usuário em uma janela construída em \textit{PyGTK}.
 \item Ao utilizar função que permite carregar um novo cenário, botão \textit{Load Scenario}, o programa
encerra os processos das máquinas do cenário anterior, finaliza a conexão com servidor telnet do \textit{uml\_switch}. O mesmo que quando acionar o botão \textit{Quit}, porém, neste caso reinicia todas
as etapas.
\end{enumerate}

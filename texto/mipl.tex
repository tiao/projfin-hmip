...
O projeto \textit{Mobile IPv6 for Linux} (MIPL) \cite{nuorvala2005mil} é uma
implementação de suporte a mobilidade no IPv6 (RFC 3775), desenvolvida na
Universidade Tecnológica de Helsinki . O código foi desenvolvido acompanhando os
...
porém, devido as substanciais mudanças na especificação do MIPv6, os gestores 
...
juntamente com o projeto USAGI \cite{usagi}. Nesta nova versão houveram
...
de Monash \cite{moore}, baseando-se no MIPL 0.94 e na versão 2.4 do 
\textit{kernel linux}.
...
como umip-0.4. O MIPL é um projeto de código aberto sobre a licença GPL versão
...
desempenha o papel de nó móvel, de agente domiciliar e de nó correspondente. A 
sua função é definida em um arquivo de configuração, sendo que todos os nós são
em potencial um nó correspondente.
...
envio de pacotes puramente IP sem a interferência do \textit{kernel}
\cite{rfc3542}.
Em condições normais quando o kernel não entende o campo de protocolo do
datagrama IP, como por exemplo, os que possuem cabeçalho de mobilidade, eles são
...
\textit{add\_task\_abs}, passando como parâme-tro:
\begin{itemize}
...
\textbf{Função mip6d} & \textbf{Mensagem Filtrada} &
\textbf{Função}\\ \hline \hline
...
domiciliar \\
	 & ICMP6\_PARAM\_PROB & Problema de Parâmetro  \\
...
tratar essas mensagens. As mensagens tratadas são: XFRM\_MSG \_ACQUIRE e
XFRM\_MSG\_REPORT.
...
É possível habilitar em tempo de compilação um terminal virtual. O terminal
...
prefixo atual.
\end{enumerate}
Devido a estas duas características, o MIPL sempre escolhe um novo roteador. Este
...
\textbf{ME\_MAP\_UPDATE} é disparado e o tempo de vida do RCoA é atualizado.
\end{enumerate}

...
O algoritimo de detecção de movimento do MIPL é todo baseado na escuta das
mensagens de anúncio de roteadores. O nó móvel sempre esta escutando as
mensagens de RA e as formas de perceber o movimento são:
\begin{itemize}
 \item caso seu roteador padrão torne-se inalcançável, o nó móvel de tempos
em tempos verifica o roteador com um \textit{Neighbor Solicitation};
 \item ou passe a receber mensagens de outro roteador e pare de escutar
mensagens do anterior.
\end{itemize}

Embora o MIPL seja uma extensão da pilha IPv6 padrão do \textit{Linux} apresenta
algumas características sobre administração de roteadores:
\begin{enumerate}
 \item O MIPL possui a sua própria lista de roteadores. Esta lista não é a
tabela normal de roteamento é atualizada de forma independente. Contém o 
atual roteador de acesso, bem como roteadores que não são atualmente
utilizados mas com tempo de vida não expirados.
 \item Outra característica do MIPL é que ele força atualizações na tabela
de roteamento após receber anúncio de novo roteador, e apaga todas as
informações de roteamento que não são do prefixo atual.
\end{enumerate}
...
\section{Alterações no MIPL}
A primeira implementação necessária foi dar suporte ao MIPL a leitura das
mensagens RA com divulgação de MAP's, pois são elas que permitem a criação do
endereço RCoA. No manipulador de mensagens RA foi adicionado o suporte para
a leitura da opção de MAP na mensagem RA. Foi criado uma estrutura de dados
...
\begin{itemize}
 ...
 \item tempo de vida do roteador que entregou a mesagem;
...
\textbf{ME\_MAP\_UPDATE}

\textbf{ME\_MAP\_EXPIRED}

Os eventos adicionados tem a função de colher os dados que serão utilizados no
momento do registro, manter os dados para o funcionamento do protocolo.

Foi criada uma estrutura de dados para o RCoA que foi adicionada na estrutura do
agente domiciliar. As informações contidas na estrutura serão utilizadas no
momento da formação das mensagens de BU e controle do registro.
\begin{lstlisting}
...
Foi adicionado ao mecanismo de registro suporte ao envio do BU para o MAP,
alteração da mensagem de BU para o agente domiciliar, criação do túnel com o MAP
e modificação do túnel com o agente domiciliar.
...
A seguir uma descrição das implementações efetuadas no MIPL em um movimento para
um novo domínio:
\begin{enumerate}
 \item Ao receber uma mensagem RA de um novo roteador de acesso:
\begin{itemize}
 \item as opções de MAP são lidas;
 \item uma uma lista de MAP's é criada na estrutura do roteador.
\end{itemize}
...
BU.
\end{itemize}
 \item movimento é detectado, código normal do MIPL, então o registro deve ser
feito com os agentes:
\begin{itemize}
...
nó móvel e MAP (ip6tnl1@ip6tnl2).
 \end{itemize}
 \item Periodicamente com a chegada dos RA's, o evento de movimento
\textbf{ME\_MAP\_UDATE} é disparado onde:
\begin{itemize}
 \item o tempo de vida do RCoA é atualizado.
\end{itemize}
\end{enumerate}

\subsection{Problemas conhecidos}
Como a implementação proposta pelo trabalho foi somente de caráter experimental,
mais como uma forma de um estudo prático dos protocolos, o tempo, alta
complexidade do código do MIPL e está em processo de desenvolvimento. Alguns
problemas possuem na implementação:
\begin{itemize}
 \item sem suporte a otimização de rotas;
 \item sem suporte a segurança (IPSec);
 \item sem suporte ao HMIPv6 em arquivo de configuração, somente em tempo de
compilação;
 \item sem tarefas para deletar MAP's expirados;
 \item problema ao receber BA vindo MAP, o que ocasiona problemas em mobilidade
em um mesmo domínio;

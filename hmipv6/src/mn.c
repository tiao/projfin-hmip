...
#ifdef HMIP
static int mn_get_map_lifetime(struct home_addr_info *hai,
				struct timespec *lifetime, int dereg)
{
	if (!hai->at_home && !dereg) {
		struct mn_addr *coa = &hai->primary_coa;
		struct mn_addr *rcoa = &hai->rai.rcoa;
		struct timespec now;
		unsigned long coa_lft;
		unsigned long rcoa_lft;

		clock_gettime(CLOCK_REALTIME, &now);		

		coa_lft = mpd_curr_lft(&now, &coa->timestamp,
				       coa->valid_time.tv_sec);
		rcoa_lft = mpd_curr_lft(&now, &rcoa->timestamp,
				       rcoa->valid_time.tv_sec);

		tssetsec(*lifetime, umin(umin(coa_lft, rcoa_lft),
					 conf.MnMaxHaBindingLife));

		mpd_sanitize_lft(lifetime);

		MDBG("CoA lifetime %u s, RCoA lifetime %u s, BU lifetime %u s\n",
		     coa_lft, rcoa_lft, lifetime->tv_sec);
				 ...
				 struct timespec *lifetime)
{
	...
	bule->coa_changed = -1;
	/* Solucao errada, somente para testar hmip */
#ifdef HMIP
	bule->coa = hai->rai.rcoa.addr;
	bule->if_coa = hai->rai.rcoa.iif;
#else
...
#ifdef HMIP
static int process_first_map_bu(struct bulentry *bule,
				 struct home_addr_info *hai,
				 struct timespec *lifetime)
{
	int err = 0;
	bule->type = BUL_ENTRY;
	bule->flags = IP6_MH_BU_HOME | IP6_MH_BU_ACK | IP6_MH_BU_MAP | hai->lladdr_comp;
	bule->coa_changed = -1;
	bule->coa = hai->primary_coa.addr;
	bule->if_coa = hai->primary_coa.iif;
	bule->lifetime = *lifetime;
	...
	hai->rai.map_reg_status = MAP_REG_UNCERTAIN;

	if ((err = mn_tnl_state_add(hai, hai->rai.if_tunnel, 0)) < 0)
		MDBG("Failed to initialize new bule for MAP\n");
	else
		MDBG("New bule for MAP\n");
...
#ifdef HMIP
	tunnel_mod(hai->if_tunnel, &hai->rai.rcoa.addr, &hai->ha_addr,
			hai->rai.rcoa.iif, mn_ext_tunnel_ops, hai);
#else
...
#ifdef HMIP
static void mn_send_map_bu(struct home_addr_info *hai)
{
	struct bulentry *bule = NULL;
	struct timespec lifetime;
	int mapreg_expired = 0;
	movement_t type_movement = MIP6_TYPE_MOVEMENT_UNKNOWN;

	TRACE;

	if (IN6_IS_ADDR_UNSPECIFIED(&hai->rai.map_addr)) {
		MDBG("MAP not set for home link\n");
		return;
	}
	mn_get_map_lifetime(hai, &lifetime, 0);

	if ((bule = bul_get(hai, NULL, &hai->rai.map_addr)) == NULL) {
		assert(!hai->at_home);
		/* Create new bul entry for MAP */
		if (!tsisset(lifetime)) {
			dbg("CoA or RCoA expired, unable to create binding.\n");
			return;
		}
		
		bule = create_bule(&hai->rai.rcoa.addr, &hai->rai.map_addr);
		if (bule == NULL)
			return;
		if (process_first_map_bu(bule, hai, &lifetime) < 0 ||
		    bul_add(bule) < 0) {
			bul_delete(bule);
			return;
		}
	...
	/* Before bul_iterate, tunnel modification should be done. */
	tunnel_mod(hai->rai.if_tunnel, &hai->primary_coa.addr, &hai->rai.map_addr,
		   hai->primary_coa.iif, NULL, NULL);
...
#endif
#ifdef HMIP
	if (!IN6_IS_ADDR_UNSPECIFIED(&hai->ha_addr) && !hai->rai.site) {
		if (hai->pend_ba)
			mn_send_home_bu(hai);
		else if (!pending_bas) {
			if (!hai->at_home ||
...
#ifdef HMIP
	tunnel_del(hai->rai.if_tunnel, NULL, NULL);
#endif
	dhaad_stop(hai);
	free(hai);
}	
...
#ifdef HMIP
static int mn_ext_map_ops(int request, int old_if, int new_if, void *data)
{
}
#endif
...
static inline void mn_update_rcoa_lifetime(struct mn_addr *mn_rcoa,
						struct map_list_entry *map)
{
	map->used = 1;
	mn_rcoa->addr = map->rcoa;
	mn_rcoa->timestamp = map->timestamp;
	tssetsec(mn_rcoa->valid_time, map->mle_valid_time);
	tssetsec(mn_rcoa->preferred_time, map->mle_valid_time);
	MDBG2("update RCoA lifetimes\n");
}
...
#endif
{
	...
	case ME_MAP_NEW:
		assert(!list_empty(&me->iface->default_rtr));
		assert(!list_empty(&rtr->maps));
		//*next_rtr = rtr;
		*map = map_choose(rtr);
		if((*map)->used) {
			MDBG("New MAP irrelevant, %d\n", (*map)->used);
			return MN_HO_IGNORE;
		}
	...
	case ME_MAP_UPDATED:
		...
		*map = map_choose(rtr);
		if(!(*map)->used) {
			MDBG("Error MAP escolhido para update não é o usado\n");
			return MN_HO_IGNORE;
		}
		return MN_HO_CHECK_RCOA_LIFETIME;
#endif
	default:
		return MN_HO_IGNORE;
	}
			      ...
			      const struct movement_event *event)
{
	...
	struct md_coa *coa = NULL;
#ifdef HMIP
	struct map_list_entry *map = NULL;
#endif
	int move_home = 0;
	...
	if (event->event_type == ME_COA_EXPIRED &&
		...
		return;
#ifdef HMIP
	hai->verdict = mn_make_ho_verdict(event, hai, &rtr, &coa, &map);
#else
	...
	if (hai->verdict == MN_HO_CHECK_LIFETIME) {
		...
		mn_coa_updated(hai);
#ifdef HMIP
	} else if (hai->verdict == MN_HO_CHECK_RCOA_LIFETIME) {
		hai->rai.rcoa.iif = hai->rai.if_tunnel;
		addr_add(&hai->rai.rcoa.addr, 128, 0, RT_SCOPE_UNIVERSE,
			hai->rai.rcoa.iif, hai->rai.rcoa.preferred_time.tv_sec,
			hai->rai.rcoa.valid_time.tv_sec);
		MDBG("update Rcoa %x:%x:%x:%x:%x:%x:%x:%x on interface (%d)\n",
				NIP6ADDR(&hai->rai.rcoa.addr), hai->rai.rcoa.iif);
#endif
		...
		case MN_HO_SITE:
			hai->rai.site = 0;
			MDBG("Change\n\n\n\n\n\n");
			if(IN6_ARE_ADDR_EQUAL(&map->mle_globaladdr, &hai->rai.map_addr)) {
				MDBG("mudança intra site\n\n\n\n\n\n");
				map->used = 1;
				hai->rai.site = 1;
				// seta alguma flag
			}

			hai->rai.map_addr = map->mle_globaladdr;
			mn_update_rcoa_lifetime(&hai->rai.rcoa, map);
	
			if(!hai->rai.if_tunnel) {
				hai->rai.if_tunnel = tunnel_add(&hai->rai.rcoa.addr, &hai->rai.map_addr, hai->if_home, NULL, NULL);

				if (hai->rai.if_tunnel <= 0) {
					MDBG("failed to create future MN-MAP tunnel\n"); 
					return;
				}
			}

...
#ifdef HMIP
static inline void md_free_router_map(struct map_list_entry *m)
{
    MDBG3("freeing map %x:%x:%x:%x:%x:%x:%x:%x/%d\n",
          NIP6ADDR(&m->mle_globaladdr), m->mle_mlen);
    list_del(&m->list);
    free(m);
}
#endif
...
#ifdef HMIP
		/* faz a mesma coisa que foi feita com os prefixos, a se pensar */
		list_for_each_safe(plist, pn, &old->maps) {
			struct map_list_entry *m;
			m = list_entry(plist, struct map_list_entry, list);

			/* don't expire maps also advertised by the
			 * new router */
			if(new && map_list_find(&new->maps,
							&m->mle_globaladdr,
							m->mle_mlen)) {
				md_free_router_map(m);
			}
		}
#endif
...
#ifdef HMIP
static struct map_list_entry *
md_create_router_map(struct md_router *rtr,
            struct nd_opt_map *map)
{
    struct map_list_entry *m;

    if(!(m = malloc(sizeof(struct map_list_entry))))
        return NULL;

	memset(m, 0, sizeof(struct map_list_entry));
	
    m->timestamp = rtr->timestamp;
    memcpy(&m->map, map, sizeof(struct nd_opt_map));
		
    /* Analisar pra ver se precisa testar alguma flag (RFC) */
			
    MDBG3("creating new map %x:%x:%x:%x:%x:%x:%x:%x/%d\n",
        NIP6ADDR(&m->mle_globaladdr), m->mle_mlen);
	...
	while (optlen > 1) {
		...
		case ND_OPT_MAP:
			if(olen < sizeof(struct nd_opt_map))
				goto free_rtr;
			map = (struct nd_opt_map *)opt;
			map->nd_opt_map_valid_time =
				ntohl(map->nd_opt_map_valid_time);

			/* Pesquisar se é necessario fazer algum teste */
            if(!(m = md_create_router_map(new, map)))
                goto free_rtr;
						
            list_add_tail(&m->list, &new->maps);
            break;
#endif
		}
		optlen -= olen;
...
#ifdef HMIP
static int md_map_cmp(struct map_list_entry *new, struct map_list_entry *old)
{
	if(IN6_ARE_ADDR_EQUAL(&new->mle_globaladdr, &old->mle_globaladdr))
		return 0;
	return -1;
}
#endif
...
static void md_update_router_stats(struct md_router *rtr)
{
	...
	list_for_each(list, &rtr->maps) {
		...
		/* pass prefix to kernel if it was included in the latest RA */
		if (!tsbefore(rtr->timestamp, m->timestamp)) {
			ipv6_addr_set(&rcoa,
				(&m->mle_globaladdr)->s6_addr32[0],
				(&m->mle_globaladdr)->s6_addr32[1],
				(&(rtr->iface)->lladdr)->s6_addr32[2],
				(&(rtr->iface)->lladdr)->s6_addr32[3]);
			MDBG("add rcoa %x:%x:%x:%x:%x:%x:%x:%x \n",
					NIP6ADDR(&rcoa));
			m->rcoa = rcoa;
		}
	}
...
#ifdef HMIP
	/* itera a lista de maps e atualiza seus parametros */
	list_for_each_safe(lnew, n, &new->maps) {
		struct map_list_entry *mnew;

		mnew = list_entry(lnew, struct map_list_entry, list);
		if (!map_list_find(&old->maps,
				      &mnew->mle_globaladdr, mnew->mle_mlen)) {
			/* a se pensar verificar as possibilidades 
			if (old->used)
				md_prefix_rule_add(mnew); */
			list_del(lnew);
			list_add_tail(lnew, &old->maps);
		} else {
			list_for_each(lold, &old->maps) {
				struct map_list_entry *mold;
				mold = list_entry(lold, 
						  struct map_list_entry,
						  list);
				if (mnew->mle_mlen != mold->mle_mlen ||
				    ipv6_pfx_cmp(&mnew->mle_globaladdr, 
						 &mold->mle_globaladdr,
						 mnew->mle_mlen))
					continue;
				mold->timestamp = mnew->timestamp;
				mold->mle_flags = mnew->mle_flags;
				mold->mle_valid_time = mnew->mle_valid_time;
				break;
			}
		}
	}
...
#ifdef HMIP
	/* TODO: Add expire task for MAP */ 
	if (!list_empty(&new->maps)) {
		__md_trigger_movement_event(ME_MAP_NEW, 0, iface, NULL);
	}
#endif
...
#ifdef HMIP
			/* TODO: Fazer comparaçao dos maps para ver se teve
			 * expire ou update ou new map */
			/* Verificar listas */
			//assert(!list_empty(&new->maps));
			//assert(!list_empty(&old->maps));
			if (!list_empty(&old->maps)) {
				//if(map_list_cmp(&new->maps, &old->maps)) {
					__md_trigger_movement_event(ME_MAP_UPDATED, 0,
									iface, NULL);
				}// else {
				//	__md_trigger_movement_event(ME_MAP_NEW, 0,
				//					iface, NULL);
				//}
			//}

...
def list():
	...
	if os.path.isdir("%s/.guml" % os.getenv("HOME")) == False:
		print "%s/.guml doesn't exist or is not a directory." %os.getenv("HOME")
		print "Please read /usr/share/doc/guml/README on how to setup guml."
		sys.exit(1)
...
def conf_route(networks):
	zebra = "! guml zebra configuration file\\nhostname Router\\npassword zebra\\nenable password zebra\\ninterface log"
	ripngd = "! guml RIPngd configuration file\\nhostname ripngd\\npassword zebra\\nrouter ripng"
...
\t/sbin/sysclt -w net.ipv6.conf.all.proxy_ndp=1\n\
...
class config:
	...
	def command_line(self):
		return ["/usr/share/guml/vm/linux.uml",
		        "mem=%sM" % self.mem,
		        "con=pts",
		        "con0=fd:0,fd:1",
	...
	def blockdevscmdline(self):
		...
		if len(self.disks) == 0:
			...
			for k in self.disks.keys():
				cmdline.append("%s=/usr/share/guml/vm/cow-%s,%s" % (k, self.umid, self.disks[k]))

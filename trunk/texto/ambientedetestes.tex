...
\item facilidades para a rápida instalação da platforma nas máquinas
hospedeiras.
\end{itemize}
...
\begin{itemize}
 \item \textit{list}: Lista as portas do switch e as VLANs a elas associadas;
 \item \textit{move} porta vlan: move uma porta do \textit{swicth} para
uma VLAN;
...
aberto ``Interface Gráfica para controle de terminais UML''
(GUML)\cite{citarGUML}, implementada com a
...
Como citado, apesar de possuir vários recursos, o GUML não fornece suporte a
configuração de cenários de rede integrado na ferramenta. O seu intuito não é
...
projeto e criar um outro um chamado ``Interface Gráfica de controle de terminais
UML para o IP Móvel'' (GUML4MIP), baseado-se no seu código fonte.
...
\textit{VLan} cada interface de rede esta associada. Caso o usuário queira
efetuar alguma mobilidade basta clicar no número da \textit{VLan} a qual queira
se associar;
...
ultimo cenário executado é salvo e na proxima execução do programa já é
...
monitora as conexões no \textit{uml\_switch}, apreentando em uma
interface gráfica o estado do \textit{switch}. Apartir desta interface p
...
sua \textit{VLan}. Seus terminais tornam-se disponíveis na interface;
\item[uml\_switch:] Processo do \textit{uml\_switch} sempre iniciado no começo
de um novo cenário de rede. Responsável por segmentar a rede em \textit{Vlan's},

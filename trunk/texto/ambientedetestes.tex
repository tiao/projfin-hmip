...
do tipo NS2 \cite{citarNS2} e Omnet \cite{citarOmnet}. A utilização destes
simuladores é interessante para a análise de desempenho dos protocolos mas não
refletem os problemas reais de configuração e execução do sistema, uma vez que
tendem a abstrair detalhes de funcionamento considerados irrelevantes na
simulação. Alé disto, as implementações do MIP e HMIP sobre estes simuladores
são bastantes precárias. O NS2 não possui nem mesmo uma versão estável do IPv6. 

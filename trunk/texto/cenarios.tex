...
implementação de cenários com fins de analisar algumas funcionalidades dos
...
com o HMIPv6. Para o primeiro cenário, duas variações foram realizadas. Na
...
disponíveis no diretório da documentação do projeto \cite{LinkProjeto}.

\section{Fundamentos para a Análise dos Cenários}
Na análise dos cenários estudados serão focados os seguintes aspectos:
\begin{itemize}
  \item a sequência e a natureza das mensagens trocadas entre os vários nós.
Neste ponto, serão utilizadas as saídas geradas pelo programa \textit{tcpdump};
...
limitações desta análise em um ambiente virtual. Os dados obtidos serão
...
foi simulada a camada enlace e, portanto, não se pode precisar com a exatidão a
latência envolvida no processo. Porém, para fins de estudo, considerar-se-á o
tempo entre a entrega do último pacote, de alguma conexão, recebido na rede
domiciliar ($t0$)e o primeiro \textit{Router Advertisement} na rede
visitada($t1$).
...
\item \textbf{Configuração do \textit{Care-of-address} (\textit{TA})}: Tempo
entre o primeiro \textit{Router Advertisement} e o envio do
\textit{Binding Update}($t2$).
...
\textit{Binding Acknowledgement}($t3$).
...
o envio das mensagens do \textit{Return Routability Procedure}($t4$) e o
recebimento do \textit{Binding Acknowledgement} do nó correspondente.
...
Outro procedimento de teste que é eventualmente realizado durante a execução dos
cenários é uma conexão utilizando os protocolos da camada transporte, por
exemplo, o TCP. Tal teste visa comprovar a transparência do processo de
...
servidor responde a requisição GET do protocolo HTTP, além de ser capaz de gerar
...
% MN & eth0 & fe:fd:32:ff:42:c9 & 2001:c::fcfd:32ff:feff:42c9 &
Auto-configurado\\
...
figura \ref{f_dump_1}. Analisando os dados recolhidos conseguimos observar:
...
\begin{enumerate}
 \item O nó móvel recebe uma mensagem de \textit{Router Advertisement} do 
roteador de acesso da rede visitada, o que indica que ocorreu mobilidade. Antes 
de realizar o movimento o nó móvel recebia \textit{echo request} do nó 
correspondente;
...
pacote. O primeiro nível IP do agente domiciliar(2001:c::1) para nó móvel
...
Os dados obtidos na saída do comando \textit{tcpdump} podem ser observados na
figura \ref{f_dump_2}. Analisando os dados recolhidos conseguimos observar:
...
processo de \textit{ruturn routability}. Note os endereços IP fonte e destinos
...
\textit{binding management key}(kbm), comcluindo o processo de 
\textit{return routability}.
%11
 \item O nó móvel recebe uma mensagem de \textit{echo request}, endereçada ao 
CoA, não passando mais pelo agente domiciliar. Note a opção DSTOPT que trás o
endereço domiciliar(2001:c::2) do nó móvel.
...
roteador de acesso (RA1) e o nó correspondente (CN). Este último irá se
comunicar com o nó móvel durante o teste;
 \item Um domínio formado pelo MAP1,que executa o \textit{daemon} MIPL na função
de agente domiciliar, e um roteador de acesso (RA2). Neste domínio
estão as sub-redes com prefixos: 2001:c::/64 (\textit{Vlan} 3) e 2001:f::/64
(\textit{Vlan} 6);
 \item Um domínio formado pelo MAP2, que executa o \textit{daemon} MIPL na
...
deste domínio as sub-redes com os prefixos: 2001:d::/64 (\textit{Vlan} 4),
2001:10::/64 (\textit{Vlan} 7) e 2001:11::/64 (\textit{Vlan} 8).
\end{itemize}
...
\textit{Vlan} 7, domínio do MAP2, caracterizando uma mobilidade global.
 \item Na segunda situação de mobilidade o nó irá migrar para a \textit{Vlan} 8
caracterizando uma mobilidade local.
 \item Neste cenário, uma mobilidade global do domínio do MAP2 para o domínio do
MAP1 também é possível (\textit{Vlan} 6). Neste caso, um novo registro como o
Agente Domiciliar é necessário. No entanto, esta operação não será explorada
aqui.
\end{enumerate}
...
\textit{echo request}s do nó correspondente (não mostrados) e mensagens RA do
...
receber mensagens RA do roteador de acesso RA3 (src= ??, endereço link local de
AR3) com a opção de MAP propagada pelo MAP2. Esta opção não é mostrada pelo
tcpdump. Com este RA o nó móvel forma os endereços LCoA() e RCoA();
 \item O nó móvel realiza o teste de duplicação de endereço (DAD) do LCoA
gerado com o prefixo da rede visitada (endereço ); 
 \item O nó móvel envia a mensagem de BU para o MAP2 (dst=) informando a dupla
(LCoA,RCoA). Note que LCoA é o  endereço fonte do pacote e o RCoA é informado
na opção DSTOPT;
\item O nó móvel envia a mensagem de BU para o Agente Domiciliar (dst=)
informando a dupla (RCoA,HoA). Note que RCoA é o endereço fonte do pacote e o
HoA é informado na opção DSTOPT;
 \item O nó móvel recebe a mensagem de BA do MAP2 como confirmação de registro
bem sucedido;
 \item O nó móvel recebe a mensagem de BA do agente domiciliar, como confirmação
de registro bem sucedido, via MAP2, de forma tunelada. Note no primeiro nível os
endereços IP do MAP () e IP LCoA(). Na sequência também chega um pacote
provindo do nó correspondente que, embora não explícito pelo
tcpdump, provavelmente pertence a um ECHO REQUEST
duplamente tunelado. Note os endereços IPs fonte e destino destes pacotes: no
primeiro nível IP do MAP para IP LCoA (src= e dst) e, no segundo nível, IP do

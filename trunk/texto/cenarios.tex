...
Após o agente domiciliar receber o \textit{Binding Update}, enviado pelo
...
Após o nó móvel receber o \textit{Router Advertisement} na rede visitada, o
...
Existem três tabelas que são criadas pelo kernel. A local(255), é uma tabela de 
roteamento especial mantida pelo kernel, onde usuários não podem adicionar novas
entradas nessa tabela. A main(254), é a tabela de roteamento principal, onde ficam
as rotas quando adicionamos com o comando \texttt{route} ou \texttt{ip route}.
A default(253) é outra tabela especial e a unspec(0), qualquer operações sobre esta
...
O Banco de Dados da Política de Roteamento ou Routing Policy Database (RPDB) 
controla a ordem na qual o kernel faz sua busca através das tabelas de roteamento.
...
\end{tabular}
\end{small}
\caption{Descrição dos \texttt{end-points} do túnel do nó móvel}
\label{t_tun_mn1}
\end{table}
...
\end{tabular}
\end{small}
\caption{Descrição dos \texttt{end-points} do túnel do agente domiciliar}
\label{t_tun_ha1}
\end{table}
...
\begin{figure}[!htpb]
	\centering
	\includegraphics[scale=.65]{figs/dump_inter}
